...
exports.inicializarBanco = void 0;
// ======================================================
...
const postos_1 = require("./postos");
const campanha_1 = require("./campanha");
...
const popularDadosReais = async () => {
    try {
        ...
        // POSTOS
        // ==================================================
        const totalPostos = await postos_1.Posto.count();
        if (totalPostos === 0) {
            await postos_1.Posto.create({
            ...
            console.log('📍 Postos de Saquarema inseridos no banco!');
        }
        // ==================================================
        // CAMPANHAS
        // ==================================================
        const totalCampanhas = await campanha_1.Campanha.count();
        if (totalCampanhas === 0) {
            await campanha_1.Campanha.create({
...
app.post('/api/usuarios/cadastro', async (req, res) => {
    try {
        ...
        const novoUsuario = await usuario_1.Usuario.create(dados);
        res.json({
            sucesso: true,
        ...
        res.status(400).json({
            sucesso: false,
            mensagem: 'Erro ao cadastrar usuário. Verifique se o CPF ou CNS já existem.'
        });
    }
});
// ---------------- LOGIN ----------------
...
app.get('/api/usuarios/:id', async (req, res) => {
    try {
        const usuario = await usuario_1.Usuario.findByPk(req.params.id);
        res.json({
            sucesso: true,
            dados: usuario
        });
...
app.get('/api/usuarios/cpf/:cpf', async (req, res) => {
    try {
        ...
        res.status(500).json({
            sucesso: false,
            mensagem: 'Erro ao buscar usuário por CPF.'
        });
    }
});
// ======================================================
...
app.get('/api/vacinas/:usuarioId', async (req, res) => {
    try {
        const vacinas = await vacina_1.Vacina.findAll({
            where: {
...
app.post('/api/vacinas', async (req, res) => {
    try {
        const novaVacina = await vacina_1.Vacina.create(req.body);
        res.json({
            sucesso: true,
...
app.get('/api/postos', async (_req, res) => {
    try {
        const postos = await postos_1.Posto.findAll();
        res.json({
            sucesso: true,
            dados: postos
        });
...
app.get('/api/campanhas', async (_req, res) => {
    try {
        const campanhas = await campanha_1.Campanha.findAll();
        res.json({
            sucesso: true,
            dados: campanhas
        });
...
app.get('/api/notificacoes/:usuarioId', async (req, res) => {
    try {
        ...
        res.status(500).json({
            sucesso: false,
            mensagem: 'Erro ao gerar notificações.'
        });
    }
});
// ======================================================
...
app.get('/api/dependentes/:usuarioId', async (req, res) => {
    try {
        const dependentes = await dependentes_1.Dependente.findAll({
            where: {
                usuarioId: req.params.usuarioId
            }
        });
        ...
        res.status(500).json({
            sucesso: false,
            mensagem: 'Erro ao buscar dependentes.'
        });
    }
});
...
app.post('/api/dependentes', async (req, res) => {
    try {
        ...
        const novoDependente = await dependentes_1.Dependente.create({
...
if (process.env.NODE_ENV !== 'production') {
    (0, exports.inicializarBanco)()
        .then(() => {
        app.listen(PORT, () => {
            console.log(`🚀 Servidor EasyVacc rodando na porta ${PORT}`);
        });
    })
        .catch((error) => {
